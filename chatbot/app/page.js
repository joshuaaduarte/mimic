  ...
  return <main className="container mx-auto max-w-4xl">
    <div className="grid grid-cols-2">
        ...
        >
          ...
          <h3 className="text-2xl text-white bold">Bot Says:</h3>
          <p className="text-white">
            {loading ? "[Bot is thinking]" : parse(marked(displayMessage))}
          </p>
        </div>

      <div>
        <Image alt="josh" src="/josh.png" width={512} height={512} />
      </div>
    </div>
    ...
    <form className="mt-6" onSubmit={handleSubmit}>
      ...
      <input className="px-4 py-2 text-gray-700 placeholder-gray-500 bg-white border border-gray-700 rounded-lg" required type="text" ref={messageRef} placeholder="Ask a question or say something nice." />
      </div>

      <button type="submit" className="px-4 py-2 mt-2 text-gray-700 bg-gray-100 border border-gray-700 rounded-lg hover:scale-110 transition-all curation-200"> Send 🚀 </button>
    </form>

    <div className="mt-6">
          {messages.map(((message) => {
            return <div key={message.content} className="flex items-center gap-4 py-2">
              <div className="w-[10%] flex items-center">
                {message.role === "assistant" ? (
                  <div className="w-[50px] h-[50px] rounded-full overflow-hidden">
                    <img src="josh.png"  className="w-full h-full object-cover"/>
                  </div>
